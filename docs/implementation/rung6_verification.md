...
- `ModRM.mod = 10`
- disp16 present
- authorized `r/m` forms:
  - `r/m=000` `[BX+SI] + disp16`
  - `r/m=001` `[BX+DI] + disp16`
  - `r/m=010` `[BP+SI] + disp16`
  - `r/m=011` `[BP+DI] + disp16`
  - `r/m=100` `[SI] + disp16`
  - `r/m=101` `[DI] + disp16`
  - `r/m=110` `[BP] + disp16`
  - `r/m=111` `[BX] + disp16`
- effective offset computed from low 16 bits plus disp16
- result wraps to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
- no segment-base addition
- no default-SS linearization
- no protected/page/segment behavior
- no full `0x67` architecture
- no broad `0x66` architecture
- no Rung 7

Implemented Pass 6H-5 MOV forms over this EA subset only:
...
Implemented Pass 6H-5 behavior:

- reuses existing MOV microcode structure after `EA_CALC_16` produces `T2`
- reuses existing `0x67` / address-size metadata from Pass 6H-1 through
  Pass 6H-4
- reuses existing decoder displacement metadata patterns from direct disp16
  and default-32 displacement handling
- reuses existing `EA_CALC_16` service ID and output contract
- extends `EA_CALC_16` only for authorized `mod=10` disp16 forms
- reads low 16 bits of existing committed 32-bit GPRs for BX, BP, SI, and DI
- uses existing single committed-GPR read plumbing
- does not add a second physical committed-GPR read port
- two-register forms sequence inside `EA_CALC_16` over the existing single
  read path
...
- preserves Pass 6H-1 direct disp16 behavior
- preserves Pass 6H-2 no-displacement non-BP behavior
- preserves Pass 6H-3 no-displacement BP-based behavior
- preserves Pass 6H-4 `mod=01` signed disp8 behavior
...
- does not add new frozen-spec fields
...
Previously implemented Pass 6H-4 addressing subset:

- `0x67` address-size override present
- 16-bit addressing only
- `ModRM.mod = 01`
- signed disp8 present
- authorized `r/m` forms:
  - `r/m=000` `[BX+SI] + signext(disp8)`
  - `r/m=001` `[BX+DI] + signext(disp8)`
  - `r/m=010` `[BP+SI] + signext(disp8)`
  - `r/m=011` `[BP+DI] + signext(disp8)`
  - `r/m=100` `[SI] + signext(disp8)`
  - `r/m=101` `[DI] + signext(disp8)`
  - `r/m=110` `[BP] + signext(disp8)`
  - `r/m=111` `[BX] + signext(disp8)`
- effective offset computed from low 16 bits plus signed disp8
- result wraps to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
- no segment-base addition
- no default-SS linearization
- no protected/page/segment behavior
- no `mod=10` disp16 16-bit addressing
- no full `0x67` architecture
- no broad `0x66` architecture
- no Rung 7

Previously implemented Pass 6H-4 MOV forms over this EA subset only:
...
Previously implemented Pass 6H-4 behavior:

- reuses existing MOV microcode structure after `EA_CALC_16` produces `T2`
- reuses existing `0x67` / address-size metadata from Pass 6H-1 through
  Pass 6H-3
- reuses existing decoder displacement metadata/sign-extension patterns
- reuses existing `EA_CALC_16` service ID and output contract
- extends `EA_CALC_16` only for authorized `mod=01` signed disp8 forms
- reads low 16 bits of existing committed 32-bit GPRs for BX, BP, SI, and DI
- uses existing single committed-GPR read plumbing
- does not add a second physical committed-GPR read port
- two-register forms sequence inside `EA_CALC_16` over the existing single
...
Previously implemented Pass 6H-3 addressing subset:

- `0x67` address-size override present
- 16-bit addressing only
- `ModRM.mod = 00`
- no displacement
- authorized `r/m` forms only:
  - `r/m=010` `[BP+SI]`
  - `r/m=011` `[BP+DI]`
- effective offset computed from low 16 bits of selected committed GPRs:
  - `[BP+SI] = BP[15:0] + SI[15:0]`
  - `[BP+DI] = BP[15:0] + DI[15:0]`
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
- two-register forms wrap to 16 bits before zero-extension
- no segment-base addition
- no default-SS linearization
- no protected/page/segment behavior
- no direct disp16 form changes:
  - `r/m=110` remains Pass 6H-1 direct disp16 behavior only
- no non-BP no-displacement changes:
  - `r/m=000/001/100/101/111` remain Pass 6H-2 behavior only
- no `mod=10` disp16 16-bit addressing
- no full `0x67` architecture
- no broad `0x66` architecture
- no Rung 7

Previously implemented Pass 6H-3 MOV forms over this EA subset only:
...
Previously implemented Pass 6H-3 behavior:

- reuses existing MOV microcode structure after `EA_CALC_16` produces `T2`
- reuses existing `0x67` / address-size metadata from Pass 6H-1 and
  Pass 6H-2
- reuses existing `EA_CALC_16` service ID and output contract
- extends `EA_CALC_16` only for authorized BP-based no-displacement forms
- reads low 16 bits of existing committed 32-bit GPRs for BP, SI, and DI
- uses existing single committed-GPR read plumbing
- does not add a second physical committed-GPR read port
- two-register forms `[BP+SI]` and `[BP+DI]` sequence inside `EA_CALC_16`
  over the existing single read path
- reuses `LOAD_RM8`, `LOAD_RM16`, and `LOAD_RM32` for memory-source reads
- reuses `STORE_RM8`, `STORE_RM16`, and `STORE_RM32` for memory-destination
  writes
- reuses `FETCH_IMM8`, `FETCH_IMM16`, and `FETCH_IMM32` for `C6/C7`
...
Previously implemented Pass 6H-2 addressing subset:
...
- authorized `r/m` forms only:
  ...
  - `r/m=111` `[BX]`
- effective offset computed from low 16 bits of selected committed GPRs:
  - `[BX+SI] = BX[15:0] + SI[15:0]`
  - `[BX+DI] = BX[15:0] + DI[15:0]`
  - `[SI] = SI[15:0]`
  - `[DI] = DI[15:0]`
  - `[BX] = BX[15:0]`
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
...
Previously implemented Pass 6H-2 MOV forms over this EA subset only:
...
Previously implemented Pass 6H-2 behavior:

- reuses existing MOV microcode structure after `EA_CALC_16` produces `T2`
- reuses existing `0x67` / address-size metadata from Pass 6H-1
- reuses existing `EA_CALC_16` service ID and output contract
- extends `EA_CALC_16` only for authorized no-displacement non-BP forms
- reads low 16 bits of existing committed 32-bit GPRs for BX, SI, and DI
- uses existing single committed-GPR read plumbing
- does not add a second physical committed-GPR read port
- two-register forms `[BX+SI]` and `[BX+DI]` sequence inside `EA_CALC_16`
  over the existing single read path
- reuses `LOAD_RM8`, `LOAD_RM16`, and `LOAD_RM32` for memory-source reads
- reuses `STORE_RM8`, `STORE_RM16`, and `STORE_RM32` for memory-destination
  writes
...
Previously implemented Pass 6H-1 addressing subset:
...
Previously implemented Pass 6H-1 MOV forms over this EA subset only:
...
Previously implemented Pass 6H-1 behavior:
...
Pass 4B, Pass 5A, and Pass 5B support:

- `0x67` + `8A /r` `MOV r8, r/m8`, memory source, 16-bit `mod=10`
  disp16 EA only
- `0x67` + `8B /r` `MOV r32, r/m32`, memory source, 16-bit `mod=10`
  disp16 EA only
- `0x66` + `0x67` + `8B /r` `MOV r16, r/m16`, memory source, 16-bit
  `mod=10` disp16 EA only
- `0x67` + `88 /r` `MOV r/m8, r8`, memory destination, 16-bit `mod=10`
  disp16 EA only
- `0x67` + `89 /r` `MOV r/m32, r32`, memory destination, 16-bit `mod=10`
  disp16 EA only
- `0x66` + `0x67` + `89 /r` `MOV r/m16, r16`, memory destination,
  16-bit `mod=10` disp16 EA only
- `0x67` + `C6 /0 ib` `MOV r/m8, imm8`, memory destination, 16-bit
  `mod=10` disp16 EA only
- `0x67` + `C7 /0 id` `MOV r/m32, imm32`, memory destination, 16-bit
  `mod=10` disp16 EA only
- `0x66` + `0x67` + `C7 /0 iw` `MOV r/m16, imm16`, memory destination,
  16-bit `mod=10` disp16 EA only

- `0x67` + `8A /r` `MOV r8, r/m8`, memory source, 16-bit `mod=01`
  signed disp8 EA only
- `0x67` + `8B /r` `MOV r32, r/m32`, memory source, 16-bit `mod=01`
  signed disp8 EA only
- `0x66` + `0x67` + `8B /r` `MOV r16, r/m16`, memory source, 16-bit
  `mod=01` signed disp8 EA only
- `0x67` + `88 /r` `MOV r/m8, r8`, memory destination, 16-bit `mod=01`
  signed disp8 EA only
- `0x67` + `89 /r` `MOV r/m32, r32`, memory destination, 16-bit `mod=01`
  signed disp8 EA only
- `0x66` + `0x67` + `89 /r` `MOV r/m16, r16`, memory destination,
  16-bit `mod=01` signed disp8 EA only
- `0x67` + `C6 /0 ib` `MOV r/m8, imm8`, memory destination, 16-bit
  `mod=01` signed disp8 EA only
- `0x67` + `C7 /0 id` `MOV r/m32, imm32`, memory destination, 16-bit
  `mod=01` signed disp8 EA only
...
- `0x67` + `8A /r` `MOV r8, r/m8`, memory source, 16-bit no-displacement
  BP-based EA only
- `0x67` + `8B /r` `MOV r32, r/m32`, memory source, 16-bit
  no-displacement BP-based EA only
- `0x66` + `0x67` + `8B /r` `MOV r16, r/m16`, memory source, 16-bit
  no-displacement BP-based EA only
- `0x67` + `88 /r` `MOV r/m8, r8`, memory destination, 16-bit
  no-displacement BP-based EA only
- `0x67` + `89 /r` `MOV r/m32, r32`, memory destination, 16-bit
  no-displacement BP-based EA only
...
- `0x67` + `C7 /0 id` `MOV r/m32, imm32`, memory destination, 16-bit
  no-displacement BP-based EA only
- `0x66` + `0x67` + `C7 /0 iw` `MOV r/m16, imm16`, memory destination,
  16-bit no-displacement BP-based EA only

- `0x67` + `8A /r` `MOV r8, r/m8`, memory source, 16-bit no-displacement
  non-BP EA only
- `0x67` + `8B /r` `MOV r32, r/m32`, memory source, 16-bit
  no-displacement non-BP EA only
- `0x66` + `0x67` + `8B /r` `MOV r16, r/m16`, memory source, 16-bit
  no-displacement non-BP EA only
- `0x67` + `88 /r` `MOV r/m8, r8`, memory destination, 16-bit
  no-displacement non-BP EA only
...
- `0x67` + `C7 /0 id` `MOV r/m32, imm32`, memory destination, 16-bit
  no-displacement non-BP EA only
- `0x66` + `0x67` + `C7 /0 iw` `MOV r/m16, imm16`, memory destination,
  16-bit no-displacement non-BP EA only
...
Date recorded: 2026-06-01 UTC.

Commands run after commit `6b25c5925cafa9ce765e914d42d649bd41949b82`:
...
| `make rung6-pass6h2-sim` | PASS |
| `make rung6-pass6h3-sim` | PASS |
| `make rung6-pass6h4-sim` | PASS |
| `make rung6-pass6h5-sim` | PASS |
...
The bounded Rung 6 Pass 6H-5 simulation proves only the authorized `0x67`
16-bit `ModRM.mod=10` disp16 effective-address slice:

- `0x67` + `8A /r` byte loads from authorized 16-bit `mod=10` disp16
  memory sources pass
- `0x67` + `8B /r` dword loads from authorized 16-bit `mod=10` disp16
  memory sources pass
- `0x66` + `0x67` + `8B /r` word loads from authorized 16-bit `mod=10`
  disp16 memory sources pass
- `0x67` + `88 /r` byte stores to authorized 16-bit `mod=10` disp16
  memory destinations pass
- `0x67` + `89 /r` dword stores to authorized 16-bit `mod=10` disp16
  memory destinations pass
- `0x66` + `0x67` + `89 /r` word stores to authorized 16-bit `mod=10`
  disp16 memory destinations pass
- `0x67` + `C6 /0 ib` byte immediate stores to authorized 16-bit `mod=10`
  disp16 memory destinations pass
- `0x67` + `C7 /0 id` dword immediate stores to authorized 16-bit `mod=10`
  disp16 memory destinations pass
- `0x66` + `0x67` + `C7 /0 iw` word immediate stores to authorized
  16-bit `mod=10` disp16 memory destinations pass
- authorized `ModRM.mod=10` forms include all eight 16-bit `r/m` encodings:
  `[BX+SI]`, `[BX+DI]`, `[BP+SI]`, `[BP+DI]`, `[SI]`, `[DI]`, `[BP]`, and
  `[BX]`, each plus disp16
- `EA_CALC_16` computes the effective offset from the low 16 bits of selected
  committed GPRs plus disp16
- effective offsets wrap to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
  for the authorized subset
- two-register forms sequence inside `EA_CALC_16` over the existing single
  committed-GPR read path
- no second physical committed-GPR read port is added
- Pass 6H-1 direct disp16 behavior remains preserved
- Pass 6H-2 no-displacement non-BP behavior remains preserved
- Pass 6H-3 no-displacement BP-based behavior remains preserved
- Pass 6H-4 `mod=01` signed disp8 behavior remains preserved
- default-32 behavior verified through Pass 6G-2 remains preserved
- bounded `0x66` behavior remains limited to authorized MOV forms only
...
- no default-SS linearization is implemented
...
- no fault occurs during the authorized bounded Pass 6H-5 MOV sequence

The existing Pass 2, Pass 4A, Pass 4B, Pass 5A, Pass 5B, Pass 6A-1,
Pass 6B-1, Pass 6C-1, Pass 6D-1, Pass 6E-1, Pass 6E-2, Pass 6E-3,
Pass 6F-1, Pass 6G-1, Pass 6G-2, Pass 6H-1, Pass 6H-2, Pass 6H-3, and
Pass 6H-4 simulations still prove preservation of:
...
- `0x67` direct disp16 memory addressing from Pass 6H-1
- `0x67` no-displacement non-BP memory addressing from Pass 6H-2
- `0x67` no-displacement BP-based memory addressing from Pass 6H-3
- `0x67` `mod=01` signed disp8 memory addressing from Pass 6H-4

Explicit Pass 6H-5 non-claims:

- this is not full Rung 6 completion
- this does not claim the full Appendix D MOV matrix
- this proves only bounded Pass 6H-5 `0x67` `mod=10` disp16 MOV support
- segment-base addition remains unsupported
- default-SS linearization remains unsupported
...
Remaining Rung 6 blockers include segment-base/default-SS/protected/page
behavior, broader MOV families, full MOV matrix verification, and final
Rung 6 acceptance.
...
Commands run after commit `1b57e6555041428232b4a8fc078b8069fdcdc8a0`:
...
- tested implementation commit:
  `1b57e6555041428232b4a8fc078b8069fdcdc8a0`
- verification was run after that implementation commit
- final `git status --short` from the run was clean
- `make codegen` and `make ucode` did not leave tracked generated drift
- this documentation update is separate from the tested implementation commit
...
The bounded Rung 6 Pass 6H-4 simulation proves only the authorized `0x67`
16-bit `ModRM.mod=01` signed disp8 effective-address slice:

- `0x67` + `8A /r` byte loads from authorized 16-bit `mod=01` signed disp8
  memory sources pass
- `0x67` + `8B /r` dword loads from authorized 16-bit `mod=01` signed disp8
  memory sources pass
- `0x66` + `0x67` + `8B /r` word loads from authorized 16-bit `mod=01`
  signed disp8 memory sources pass
- `0x67` + `88 /r` byte stores to authorized 16-bit `mod=01` signed disp8
  memory destinations pass
- `0x67` + `89 /r` dword stores to authorized 16-bit `mod=01` signed disp8
  memory destinations pass
- `0x66` + `0x67` + `89 /r` word stores to authorized 16-bit `mod=01`
  signed disp8 memory destinations pass
- `0x67` + `C6 /0 ib` byte immediate stores to authorized 16-bit `mod=01`
  signed disp8 memory destinations pass
- `0x67` + `C7 /0 id` dword immediate stores to authorized 16-bit `mod=01`
  signed disp8 memory destinations pass
- `0x66` + `0x67` + `C7 /0 iw` word immediate stores to authorized
  16-bit `mod=01` signed disp8 memory destinations pass
- authorized `ModRM.mod=01` forms include all eight 16-bit `r/m` encodings:
  `[BX+SI]`, `[BX+DI]`, `[BP+SI]`, `[BP+DI]`, `[SI]`, `[DI]`, `[BP]`, and
  `[BX]`, each plus sign-extended disp8
- `EA_CALC_16` computes the effective offset from the low 16 bits of selected
  committed GPRs plus signed disp8
- effective offsets wrap to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
  for the authorized subset
- two-register forms sequence inside `EA_CALC_16` over the existing single
...
- no full `0x67` architecture is implemented
- no broad `0x66` architecture is implemented
- no fault occurs during the authorized bounded Pass 6H-4 MOV sequence

The existing Pass 2, Pass 4A, Pass 4B, Pass 5A, Pass 5B, Pass 6A-1,
Pass 6B-1, Pass 6C-1, Pass 6D-1, Pass 6E-1, Pass 6E-2, Pass 6E-3,
Pass 6F-1, Pass 6G-1, Pass 6G-2, Pass 6H-1, Pass 6H-2, and Pass 6H-3
...
Explicit Pass 6H-4 non-claims:

- this is not full Rung 6 completion
- this does not claim the full Appendix D MOV matrix
- this proves only bounded Pass 6H-4 `0x67` `mod=01` signed disp8 MOV support
...
Remaining Rung 6 blockers include 16-bit `mod=10` disp16,
segment-base/default-SS/protected/page behavior, broader MOV families, full
...
Date recorded: 2026-05-31 UTC.

Commands run after commit `f714d278f2ef0a86e0ea96ca8e2e56ef8843db57`:
...
- tested implementation commit:
  `f714d278f2ef0a86e0ea96ca8e2e56ef8843db57`
- verification was run after that implementation commit
- final `git status --short` from the run was clean
- `make codegen` and `make ucode` did not leave tracked generated drift
- this documentation update is separate from the tested implementation commit
...
The bounded Rung 6 Pass 6H-3 simulation proves only the authorized `0x67`
16-bit no-displacement BP-based effective-address slice:

- `0x67` + `8A /r` byte loads from authorized 16-bit no-displacement
  BP-based memory sources pass
- `0x67` + `8B /r` dword loads from authorized 16-bit no-displacement
  BP-based memory sources pass
- `0x66` + `0x67` + `8B /r` word loads from authorized 16-bit
  no-displacement BP-based memory sources pass
- `0x67` + `88 /r` byte stores to authorized 16-bit no-displacement
  BP-based memory destinations pass
- `0x67` + `89 /r` dword stores to authorized 16-bit no-displacement
  BP-based memory destinations pass
- `0x66` + `0x67` + `89 /r` word stores to authorized 16-bit
  no-displacement BP-based memory destinations pass
- `0x67` + `C6 /0 ib` byte immediate stores to authorized 16-bit
  no-displacement BP-based memory destinations pass
- `0x67` + `C7 /0 id` dword immediate stores to authorized 16-bit
  no-displacement BP-based memory destinations pass
- `0x66` + `0x67` + `C7 /0 iw` word immediate stores to authorized 16-bit
  no-displacement BP-based memory destinations pass
- authorized `ModRM.mod=00` forms are limited to `r/m=010` `[BP+SI]` and
  `r/m=011` `[BP+DI]`
- `EA_CALC_16` computes the effective offset from the low 16 bits of the
  selected committed GPRs
- `[BP+SI]` and `[BP+DI]` wrap to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
  for the authorized subset
- two-register forms sequence inside `EA_CALC_16` over the existing single
  committed-GPR read path
...
- no full `0x67` architecture is implemented
- no broad `0x66` architecture is implemented
- no fault occurs during the authorized bounded Pass 6H-3 MOV sequence

The existing Pass 2, Pass 4A, Pass 4B, Pass 5A, Pass 5B, Pass 6A-1,
Pass 6B-1, Pass 6C-1, Pass 6D-1, Pass 6E-1, Pass 6E-2, Pass 6E-3,
Pass 6F-1, Pass 6G-1, Pass 6G-2, Pass 6H-1, and Pass 6H-2 simulations still
...
Explicit Pass 6H-3 non-claims:

- this is not full Rung 6 completion
- this does not claim the full Appendix D MOV matrix
- this proves only bounded Pass 6H-3 `0x67` no-displacement BP-based MOV
  support
...
Remaining Rung 6 blockers include 16-bit `mod=01` signed disp8, 16-bit
`mod=10` disp16, segment-base/default-SS/protected/page behavior, broader MOV
...
Date recorded: 2026-05-28 UTC.

Commands run after commit `631ef89ce335d7ffcc157aca72c47e1fc35e7240`:
...
- tested implementation commit:
  `631ef89ce335d7ffcc157aca72c47e1fc35e7240`
- verification was run after that implementation commit
- final `git status --short` from the run was clean
- `make codegen` and `make ucode` did not leave tracked generated drift
- this documentation update is separate from the tested implementation commit
...
The bounded Rung 6 Pass 6H-2 simulation proves only the authorized `0x67`
16-bit no-displacement non-BP effective-address slice:

- `0x67` + `8A /r` byte loads from authorized 16-bit no-displacement non-BP
  memory sources pass
- `0x67` + `8B /r` dword loads from authorized 16-bit no-displacement non-BP
  memory sources pass
- `0x66` + `0x67` + `8B /r` word loads from authorized 16-bit
  no-displacement non-BP memory sources pass
- `0x67` + `88 /r` byte stores to authorized 16-bit no-displacement non-BP
  memory destinations pass
- `0x67` + `89 /r` dword stores to authorized 16-bit no-displacement non-BP
  memory destinations pass
- `0x66` + `0x67` + `89 /r` word stores to authorized 16-bit
  no-displacement non-BP memory destinations pass
- `0x67` + `C6 /0 ib` byte immediate stores to authorized 16-bit
  no-displacement non-BP memory destinations pass
- `0x67` + `C7 /0 id` dword immediate stores to authorized 16-bit
  no-displacement non-BP memory destinations pass
- `0x66` + `0x67` + `C7 /0 iw` word immediate stores to authorized 16-bit
  no-displacement non-BP memory destinations pass
- authorized `ModRM.mod=00` forms are limited to `r/m=000` `[BX+SI]`,
  `r/m=001` `[BX+DI]`, `r/m=100` `[SI]`, `r/m=101` `[DI]`, and `r/m=111`
  `[BX]`
- `EA_CALC_16` computes the effective offset from the low 16 bits of the
  selected committed GPRs
- `[BX+SI]` and `[BX+DI]` wrap to 16 bits before zero-extension
- `EA_CALC_16` writes `T2 = zero-extended wrapped 16-bit effective offset`
  for the authorized subset
- two-register forms sequence inside `EA_CALC_16` over the existing single
  committed-GPR read path
...
- no new frozen-spec field, opcode class, service ID, commit mask, or
  microinstruction opcode is used
- EFLAGS remain unchanged
- no BP-involving 16-bit addressing is implemented
- no 16-bit `mod=01` signed disp8 addressing is implemented
- no 16-bit `mod=10` disp16 addressing is implemented
- no segment-base addition is implemented
- no protected/page/segment behavior is implemented
- no full `0x67` architecture is implemented
- no broad `0x66` architecture is implemented
- no fault occurs during the authorized bounded Pass 6H-2 MOV sequence

The existing Pass 2, Pass 4A, Pass 4B, Pass 5A, Pass 5B, Pass 6A-1,
Pass 6B-1, Pass 6C-1, Pass 6D-1, Pass 6E-1, Pass 6E-2, Pass 6E-3,
Pass 6F-1, Pass 6F-2, Pass 6G-1, Pass 6G-2, and Pass 6H-1 simulations still
prove preservation of:
...
Explicit Pass 6H-2 non-claims:

- this is not full Rung 6 completion
- this does not claim the full Appendix D MOV matrix
- this proves only bounded Pass 6H-2 `0x67` no-displacement non-BP MOV support
- BP-based 16-bit addressing remains unsupported
- 16-bit `mod=01` signed disp8 addressing remains unsupported
- 16-bit `mod=10` disp16 addressing remains unsupported
- segment-base addition remains unsupported
- protected/page/segment behavior remains unsupported
...
Remaining Rung 6 blockers include BP-based 16-bit addressing, 16-bit
`mod=01` signed disp8, 16-bit `mod=10` disp16, segment-base/protected/page
