...
### Test 8 — Decode Stability
**Proves:** ENTRY_NULL is never dispatched in a pure NOP stream.
**Pass:** no decode_done with entry_id=0x00 after Test 1 fires.

## Expected Output

    --- Reset released, Rung 1 NOP loop starting ---
    PASS Test 1: decoder classified 0x90 as ENTRY_NOP_XCHG_AX (0x13)
    PASS Test 2: uPC reached 0x020 (ENTRY_NOP_XCHG_AX dispatch address)
    PASS Test 3: no fault during NOP execution (fault_pending=0)
    PASS Test 4: EIP advanced by 1 after NOP (0xFFFFFFF0 -> 0xFFFFFFF1)
    PASS Test 5: microsequencer returned to FETCH_DECODE after NOP
    PASS Test 6: 10 consecutive NOPs completed (fault_count=0)
    PASS Test 7: 100 consecutive NOPs completed, zero spurious faults
    PASS Test 8: ENTRY_NULL never dispatched during NOP stream (decode stable)
