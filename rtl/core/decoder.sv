...
// rtl/core/decoder.sv
//
// Decoder role through Rung 6 Pass 6C-1:
...
//     register-base, disp8, mod=01/10, and 0x67 remain unsupported.
...
//     and report M_NEXT_EIP after the immediate. For Pass 6D-1, also accept
//     C6/C7 /0 with ModRM.mod=11 as register-destination MOV and report
//     M_NEXT_EIP after the immediate. FETCH_IMM* remains the microcode-called
//     service that consumes the immediate payload.
    ...
    function automatic logic [7:0] classify_opcode(
        ...
        if (pref66) begin
            case (op)
                ...
                8'h8B: return (mov_modrm_reg || mov_mem_src_disp32) ? ENTRY_MOV
                                                                     : ENTRY_NULL;
                8'hC7: return (mov_rm_imm_disp32 || mov_rm_imm_reg) ? ENTRY_MOV
                                                                     : ENTRY_NULL;
        ...
        case (op)
            ...
            8'h8A, 8'h8B:     return (mov_modrm_reg || mov_mem_src_disp32) ? ENTRY_MOV
                                                                            : ENTRY_NULL;
            8'hC6, 8'hC7:     return (mov_rm_imm_disp32 || mov_rm_imm_reg) ? ENTRY_MOV
                                                                            : ENTRY_NULL;
